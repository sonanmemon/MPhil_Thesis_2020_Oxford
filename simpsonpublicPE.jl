...
function simpsonpublicPE(j::Number, p::Number, mminus::Array, zh::Number, zl::Number, N::Number, zk::Number, shat::Number, m::Number, sigmaeta::Number, sigmaepsilon::Number, sigmas::Number)
    ...
    f = funcetapdfupdated(j,eta)*pdf(k, (m - cdf(x, (s - (zk+eta))/(sigmas))))
    return f
     end
